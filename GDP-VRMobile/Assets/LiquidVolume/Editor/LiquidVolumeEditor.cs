...
namespace LiquidVolumeFX
{
	...
	public class LiquidVolumeEditor : Editor
	{
		...
		string[] shaderPaths;
		bool shaderFeatureScattering, shaderFeatureSmoke, shaderFeatureFPRenderTextures;
			...
			if (detailed != (int)DETAIL.Smoke) {
				...
				if (expandSection [LIQUID_SETTINGS]) {
					...
					if (refractionBlur.boolValue) {
						...
						if (distortionMap.objectReferenceValue != null) {
		...
		void DisconnectPrefabInstance(GameObject gameObject) {
			...
			if (prefabInstanceStatus != UnityEditor.PrefabInstanceStatus.NotAPrefab) {
				gameObject = GetRootGameObject(gameObject);
        ...
        string GetUniqueFilePath(string path) {
            while (File.Exists(path)) {
				...
				path = "Assets/" + filename + "_" + extension;
            }
			return path;
        }
